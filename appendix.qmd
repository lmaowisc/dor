...
$$
S_D(t) + n^{-1}\hat{\boldsymbol\eta}(t)^{\rm T}\boldsymbol Z, \mbox{ok}
$$
...
$\mathcal M(\hat S_D)$ by that of $\mathcal M\left\{\hat S_D(\cdot) + n^{-1}\hat{\boldsymbol\eta}(\cdot)^{\rm T}\boldsymbol Z\right\}$ using repeatedly generated $\boldsymbol Z$
